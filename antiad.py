...
def recv():
    while True:
        ...
        if posttype == 'message':
            ...
            if (msgtype == 'group') \
              and (gid in GROUPS) \
              and (f'[CQ:at,qq={msg.get("self_id")}' in text) \
              and (uid not in NOOPERATORS):  # 在管理群内被@且发送者未被屏蔽
                aderqq = int(text.split()[1])
                blacklist(aderqq,f'[CQ:reply,id={msgid}] 已移除[CQ:at,qq={aderqq}]')
        print(msg)
